...
> G0011 当前只完成代码、文档、本地 Debug 自动化和 Standalone 烟雾。以下正式 ZIP、干净部署与真实 Host
> 流程保留为发布阶段操作，本轮没有执行，也不计入本地完成门禁。
...
<!-- 2、3. src/FractalArtPlugin.Plugin/FractalArtPlugin.Plugin.csproj：引用并声明由插件携带 -->
<ItemGroup>
...
- 用 `dotnet list src/FractalArtPlugin.Plugin/FractalArtPlugin.Plugin.csproj package --include-transitive` 查看传递依赖；
...
该目标只重建 `Controls/FractalArtPlugin`，不会清理 `Controls` 根目录或其他插件。需要给开发目录增加醒目标记时，
可覆盖目录名：
...
dotnet msbuild src/FractalArtPlugin.Plugin/FractalArtPlugin.Plugin.csproj `
  ...
  -p:Configuration=Debug `
  -p:ManagedPluginDeployRoot=C:\Path\To\Host\Controls `
  -p:ManagedPluginDirectoryName=FractalArtPlugin-Dev
```

`FractalArtPlugin-Dev` 只是文件夹名称，插件身份仍是 manifest 中的 `myavalonia.plugin.fractal.art`。
...
dotnet msbuild src/FractalArtPlugin.Plugin/FractalArtPlugin.Plugin.csproj `
  -t:DeployManagedPlugin `
  -p:Configuration=Debug `
  -p:ManagedPluginDeployRoot=C:\Temp\FractalArtPlugin-Deploy\Controls
```

然后把整个 `C:\Temp\FractalArtPlugin-Deploy\Controls\FractalArtPlugin` 复制到 Host 的 `Controls` 下。目标叶子目录可以
改为 `FractalArtPlugin-Dev`，但必须遵守：

- 把插件目录作为一个整体替换，不要把新文件合并覆盖到旧目录，否则删除过的依赖可能残留；
- 同一个 Host 中只保留一份 `myavalonia.plugin.fractal.art`，不能同时留下 `FractalArtPlugin` 和 `FractalArtPlugin-Dev`；
...
```text
src/FractalArtPlugin.Plugin/artifacts/managed-plugin-packages/
├─ FractalArtPlugin.Plugin-1.0.0-win-x64.zip
└─ FractalArtPlugin.Plugin-1.0.0-win-x64.manifest.json
```

ZIP 内保持 `Controls/FractalArtPlugin/` 布局；同名外置 `.manifest.json` 记录 ZIP 和文件摘要。正式交付时让二者
